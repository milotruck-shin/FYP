...
/* Definitions for cq_task */
osThreadId_t cq_taskHandle;
const osThreadAttr_t cq_task_attributes = {
  .name = "cq_task",
...
/* Definitions for query_mux */
osMutexId_t query_muxHandle;
const osMutexAttr_t query_mux_attributes = {
  .name = "query_mux"
};
...
void CMD_Q(void *argument);
void State_machine(void *argument);
void UART_commands(void *argument);
...
int main(void)
{
  ...
  osKernelInitialize();
  /* Create the mutex(es) */
  /* creation of query_mux */
  query_muxHandle = osMutexNew(&query_mux_attributes);
  ...
  /* Create the thread(s) */
  /* creation of cq_task */
  cq_taskHandle = osThreadNew(CMD_Q, NULL, &cq_task_attributes);

  /* creation of sm_task */
  sm_taskHandle = osThreadNew(State_machine, NULL, &sm_task_attributes);

  /* creation of uart_task */
  uart_taskHandle = osThreadNew(UART_commands, NULL, &uart_task_attributes);
...
static void MX_FDCAN2_Init(void)
{
  ...
  hfdcan2.Init.NominalSyncJumpWidth = 57;
  hfdcan2.Init.NominalTimeSeg1 = 112;
  hfdcan2.Init.NominalTimeSeg2 = 57;
    ...
    if (huart->Instance == LPUART1)
    {
        ...
        if (rx_data_SERIAL == '\r' || rx_data_SERIAL == '\n')
        {
            ...
            if (rx_index > 0)
            {
                ...
                command_ready = 1;
                osEventFlagsSet(myEvent01Handle, UART_EVENT);
...
void HAL_TIM_OC_DelayElapsedCallback(TIM_HandleTypeDef *htim)
{
	if (htim==&htim4)
	{
	    if(htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
	        // Ch1 interrupt (5ms)
		  osEventFlagsSet(myEvent01Handle, CQ_EVENT);
		  TIM4->CCR1 += 500;

	    }
	    if(htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2) {
	        // Ch2 interrupt (40ms)
		  osEventFlagsSet(myEvent01Handle, SM_EVENT);
		  TIM4->CCR2 += 4000;
...
void motor_init(void)
{
    ...
    moteus_can_init(&hfdcan2);

    // Create motor instance (ID 1)
    motor = moteus_init(&hfdcan2, 2);
    if (!motor) {
...
/* USER CODE BEGIN Header_CMD_Q */
/**
  * @brief  Function implementing the cq_task thread.
  * @param  argument: Not used
  * @retval None
  */
/* USER CODE END Header_CMD_Q */
void CMD_Q(void *argument)
{
  /* USER CODE BEGIN 5 */
  /* Infinite loop */
	for(;;)
	{
		osEventFlagsWait(myEvent01Handle, CQ_EVENT, osFlagsWaitAll, osWaitForever);
		if (!stopped) {
	              osMutexAcquire(query_muxHandle, osWaitForever);
...
void State_machine(void *argument)
{
  ...
  /* Infinite loop */
	...
	for(;;)
	{
		osEventFlagsWait(myEvent01Handle, SM_EVENT, osFlagsWaitAll, osWaitForever);
        osMutexAcquire(query_muxHandle, osWaitForever);
        if (r != NULL) {
            float torque = r->torque;
            if (torque > 2.0f) {
...
/* USER CODE BEGIN Header_UART_commands */
/**
* @brief Function implementing the uart_task thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_UART_commands */
void UART_commands(void *argument)
{
  /* USER CODE BEGIN UART_commands */
  /* Infinite loop */

  for(;;)
  {
	  osEventFlagsWait(myEvent01Handle, UART_EVENT, osFlagsWaitAll, osWaitForever);
	  if (command_ready)
	  {
	      command_ready = 0;
	      char *endptr;
	      ...
	      else if (strncmp(HLPUART1_rxBuffer, "START", 5) == 0)
	      {
	          HAL_GPIO_WritePin(GPIOC, GPIO_PIN_12, GPIO_PIN_RESET);
	          osMutexAcquire(query_muxHandle, osWaitForever);
              cmd.position = 0.0;
